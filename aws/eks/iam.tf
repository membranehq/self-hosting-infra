...
resource "aws_iam_role" "integration_app_sa" {
  name = "${var.project}-${var.environment}-sa-role"
...
resource "aws_iam_role_policy" "integration_app_s3" {
  name = "${var.project}-${var.environment}-s3-policy"
  role = aws_iam_role.integration_app_sa.id
...
# Policy for External DNS to manage Route53
